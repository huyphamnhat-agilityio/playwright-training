...
test.describe("Authentication Test Suite", () => {
  ...
  test("TC_AUTH_002: User cannot login with wrong username and password", async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    ...
    // Steps:
    // 1. User focuses the username field
    // 2. User fills the username field with value: "standard_user2"
    await loginPage.usernameInput.click();
    await loginPage.usernameInput.fill("standard_user2");

    // 3. User focuses the password field
    // 4. User fills the password field with value: "secret_sauce2"
    await loginPage.passwordInput.click();
    await loginPage.passwordInput.fill("secret_sauce2");

    // 5. User clicks the login button
    await loginPage.loginButton.click();

    // Expected Result: User stays in login page and see the error message in the login form
